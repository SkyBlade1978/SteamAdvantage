		...
		if(isServerWorld){
			if(!redstone){
				...
				if(timeUntilNextPump == 0 && getTank().getFluidAmount() <= 0){
					...
					while(target.getY() > 0 && w.getBlockState(target).getBlock() == com.mcmoddev.steamadvantage.init.Blocks.pump_pipe_steam){
						target = target.down();
					}
					if(target.getY() > 0 && w.isAirBlock(target) && getEnergy(Power.steam_power) >= ENERGY_COST_PIPE){
						// place pipe
						w.setBlockState(target, com.mcmoddev.steamadvantage.init.Blocks.pump_pipe_steam.getDefaultState());
