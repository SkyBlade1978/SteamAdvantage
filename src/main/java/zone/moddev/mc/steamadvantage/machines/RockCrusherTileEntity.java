...
public class RockCrusherTileEntity extends cyano.poweradvantage.api.simple.TileEntitySimplePowerMachine{
	...
	public RockCrusherTileEntity() {
		super(Power.steam_power, 50, "tile.steamadvantage.steam_crusher.name");
	...
	public void tickUpdate(boolean isServerWorld) {
		if(isServerWorld){
			...
			if(redstone){
				...
				ItemStack output = getCrusherRecipeOutput(inventory[0]);
				// reset progress if item is not crushable
				if(progress > 0 && output == null){
					progress = 0;
				}
	...
	private ItemStack getCrusherRecipeOutput(ItemStack item) {
		if(item == null) return null;
		return BaseMetalsCompat.getCrusherRecipeOutput(item);
	}
	...
	public int getComparatorOutput() {
		...
		return Math.min(1 + 14 * inventory[0].stackSize / inventory[0].getMaxStackSize(), 15);
	}
