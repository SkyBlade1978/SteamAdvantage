...
public class ElectricBoilerBlock  extends cyano.poweradvantage.api.simple.BlockSimplePowerMachine{
	...
	public void onBlockAdded(World w, BlockPos coord, IBlockState state){
		...
		ConduitRegistry.getInstance().conduitBlockPlacedEvent(w, w.provider.getDimension(), coord, zone.moddev.mc.poweradvantage.init.Fluids.fluidConduit_general);
	}
	...
	public void onBlockDestroyedByPlayer(World w, BlockPos coord, IBlockState state){
		...
		ConduitRegistry.getInstance().conduitBlockRemovedEvent(w, w.provider.getDimension(), coord, ElectricBoilerTileEntity.ELECTRIC_POWER);
